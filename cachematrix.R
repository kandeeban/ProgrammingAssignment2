...
# there may be some benefit to caching the inverse of a matrix rather than compute it repeatedly. The
...
x<-rbind(c(1.729,-0.8),c(-0.8,1.729))
mcm<-makeCacheMatrix(x)
mcm$get()

#first time run i.e. no cached data
cacheSolve(mcm)

#getting cached data here
cacheSolve(mcm)
